...
def detect():
    ...
    #             break
    if auto_move:
        if not snake['x'] == fruit['x']:
            if not check_collision_auto_x(1):
                snake['dir'] = "right"
            elif not check_collision_total_auto(-1):
                snake['dir'] = "left"
            else:
                snake['dir'] = "stop"
        else:
            if not check_collision_auto_y(1):
                snake['dir'] = "down"
            elif not check_collision_auto_y(-1):
                snake['dir'] = "up"
            else:
                snake['dir'] = "stop"
        # if snake['x'] < fruit['x']:
...
def check_collision_auto_x(dir):
    for i in range(snake['length']):
        if (snake['x']) + dir == tailX[i]:
            return True

def check_collision_auto_y(dir):
    for i in range(snake['length']):
        if (snake['y'] + dir) == tailY[i]:
            return True

def check_collision_total_auto(dir):
    if check_collision_auto_x(dir) or check_collision_auto_y(dir):
        return True
