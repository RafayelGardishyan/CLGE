...
"""
This module is experimental and may cause errors or bugs
...
class Physiqs:
    ...
    def appendForces(self):
        ...
        self.frame = Vector2(0, self.GetGravityForce() * self.screen.fixedUpdateTimout)
        for i in range(len(self.forces)):
            self.frame += Vector2(self.forces[i].x * self.screen.fixedUpdateTimout, self.forces[i].y * self.screen.fixedUpdateTimout)
    ...
    def updateGravityFrame(self):
        self.gravityFrame += 1 * self.screen.fixedUpdateTimout
    ...
    def checkReset(self):
        if self.transform2d.blockMovement["down"]:
            self.addForce(Vector2(0, -1), 5 / self.mass)
            self.gravityFrame = 1
